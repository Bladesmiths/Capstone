	...
	SubShader
	{
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput inputMesh  )
			{
				...
				float3 vertexPos80 = inputMesh.positionOS;
				float4 ase_clipPos80 = TransformWorldToHClip( TransformObjectToWorld(vertexPos80));
				float4 screenPos80 = ComputeScreenPos( ase_clipPos80 , _ProjectionParams.x );
			...
			float4 Frag( VertexOutput packedInput ) : SV_Target
			{
				...
				float4 screenPos80 = packedInput.ase_texcoord2;
				...
				float screenDepth80 = LinearEyeDepth(SampleCameraDepth( ase_screenPosNorm80.xy ),_ZBufferParams);
		...
		Pass
		{
					...
					)
			{
				...
				float4 tex2DNode9 = tex2D( _Diffuse, uv_Diffuse );
				float4 screenPos80 = packedInput.ase_texcoord1;
				float4 ase_screenPosNorm80 = screenPos80 / screenPos80.w;
				ase_screenPosNorm80.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm80.z : ase_screenPosNorm80.z * 0.5 + 0.5;
				float screenDepth80 = LinearEyeDepth(SampleCameraDepth( ase_screenPosNorm80.xy ),_ZBufferParams);
				float distanceDepth80 = abs( ( screenDepth80 - LinearEyeDepth( ase_screenPosNorm80.z,_ZBufferParams ) ) / ( _SoftParticleFactor ) );
		...
		Pass
		{
					...
					)
			{
				...
				float4 tex2DNode9 = tex2D( _Diffuse, uv_Diffuse );
				float4 screenPos80 = packedInput.ase_texcoord4;
				float4 ase_screenPosNorm80 = screenPos80 / screenPos80.w;
				ase_screenPosNorm80.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm80.z : ase_screenPosNorm80.z * 0.5 + 0.5;
				float screenDepth80 = LinearEyeDepth(SampleCameraDepth( ase_screenPosNorm80.xy ),_ZBufferParams);
				float distanceDepth80 = abs( ( screenDepth80 - LinearEyeDepth( ase_screenPosNorm80.z,_ZBufferParams ) ) / ( _SoftParticleFactor ) );
...
/*ASEBEGIN
Version=18901
826;150;1601;1338;565.998;499.2113;1;True;True
...
ASEEND*/
//CHKSM=C49CB44B7BD887C4B5200E4BEE8DCC9887828527